...
class DBOpenHelper(context: Context?) : SQLiteOpenHelper(context, DBNAME, null, DB_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val sql =
            "CREATE TABLE ${ProviderConstant.TABLE_NAME} (${ProviderConstant.COLUMN_ID} INTEGER primary key autoincrement,${ProviderConstant.COLUMN_PACKAGE_NAME} TEXT, ${ProviderConstant.COLUMN_ALLOW_SHOW} TEXT)"
        db.execSQL(sql)
    }
