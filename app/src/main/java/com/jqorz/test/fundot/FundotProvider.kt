...
import com.jqorz.common.fundot.ProviderConstant


class FundotProvider : ContentProvider() {
    companion object {
        ...
        init {
            MATCHER.addURI(ProviderConstant.AUTHORITIES, ProviderConstant.PATH_PACKAGE_STATUS, CODE_USER_INFO_DIR)
            //#号为通配符
            MATCHER.addURI(ProviderConstant.AUTHORITIES, ProviderConstant.PATH_PACKAGE_STATUS + "/#", CODE_USER_INFO_ITEM)
        }
    }
    ...
    override fun query(
        ...
        return when (MATCHER.match(uri)) {
            ...
            CODE_USER_INFO_ITEM -> {
                db.query(
                    ProviderConstant.TABLE_NAME,
                    projection,
                    appendIdParamsIfNeed(uri, selection),
                    selectionArgs,
                    null,
                    null,
                    sortOrder
    ...
    override fun getType(uri: Uri): String {
        return when (MATCHER.match(uri)) {
            CODE_USER_INFO_DIR -> "vnd.android.cursor.dir/${ProviderConstant.PATH_PACKAGE_STATUS}"
            CODE_USER_INFO_ITEM -> "vnd.android.cursor.item/${ProviderConstant.PATH_PACKAGE_STATUS}"
            else -> throw  IllegalArgumentException("Unknown Uri:$uri")
        }
    }
}